...
var PLATFORM_DASH_1 = preload("res://Scenes/black_hole.tscn")
...
func create_platform(clearance):
	if clearance:
		var pos = get_viewport().get_mouse_position() + rigid_body_2d.position - (screen_size/2)
...
func _process(delta):
	...
	check_for_clearance()
	change_icon()
	clearance_checker.position = get_viewport().get_mouse_position() + rigid_body_2d.position  - (screen_size/2) 
